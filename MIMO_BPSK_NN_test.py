...
from CommPy.detector import MMSE_BPSK
...
ckpt_load_path = './checkpoints/MIMO_BPSK_Tx4_Rx4_Adam/FixH_MMSEdet_CH9_epoch1.pt'
...
def single_time_transmit(x_symbols, var_noise, H, constellation, Net, cuda):
    # compute noise
    ...
    """ detection """
    xhat_indices = apply_network(y_equalized, Net, cuda=False)

    return y_equalized, xhat_indices
...
def main():
    ...
    # build neural network and load checkpoint
    ...
    for dd in range(0, SNRdB_range.shape[0]):
        ...
        for bb in tqdm(range(0, num_packets)):
            ...
            for ll in range(0, new_length):
                y_equalized, xhat_indices = single_time_transmit(x_symbols_array[:, ll], var_noise, H, constellation, model, cuda=False)
                xhat_indices_array[:, ll] = xhat_indices

            # demodulate, decode and CRC, if one user fails, store the info for all users
            pass_flag = 1
            for kk in range(0, N):
                xhat_symbols = constellation[xhat_indices_array[kk]]
                SER_uncoded_block[kk, bb] = bit_err_rate(x_indices_array[kk], xhat_indices_array[kk])
