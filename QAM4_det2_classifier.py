...
from dataset import QAM4_Symbol2Bits
...
def apply_network(y_symbols, Net, cuda):
    
    xhat_indices = np.zeros(y_symbols.shape, dtype=int)
    if cuda:
        for i in range(0, y_symbols.shape[0]):
            y_cat = np.array([np.real(y_symbols[i]), np.imag(y_symbols[i])])
            y_tensor = torch.from_numpy(y_cat).float()
            y_tensor = y_tensor.cuda()
            xhat_tensor = Net(y_tensor).cpu().data()
            xhat_indices[i] = torch.argmax(xhat_tensor.detach()).item()
    else:
        for i in range(0, y_symbols.shape[0]):
            y_cat = np.array([np.real(y_symbols[i]), np.imag(y_symbols[i])])
            y_tensor = torch.from_numpy(y_cat).float()
...
def eval_once(storage_path, params, Net, modem, trellis, pass_rate, cuda=False):
    ...
    for k in tqdm(range(1, datalen)):
        ...
        x_indices = x_indices_array[k]
        remainder, codeword = CRC_encode(message_bits, params['CRC_key'])
        y_symbols = y_array[k]

        xhat_indices = apply_network(y_symbols, Net, cuda)
        xhat_symbols = modem.constellation[xhat_indices]         
...
def train_eval(storage_path, pass_rate, ckpt_load_path, ckpt_save_path, cuda):
    ...
    training_data = QAM4_Symbol2Bits(storage_path)
...
def main():
    storage_path = './filtered_data/QAM4_Tx100_Rx100_FlatFading_bound5e-1_bound5e-1.npy'   
    #storage_path = './filtered_data/test.npy'  
    pass_rate = np.array([0.018, 0.076, 0.2, 0.327, 0.453, 0.676, 0.73, 0.839, 0.867])   
    ckpt_load_path = None
    ckpt_save_path = './checkpoints/QAM4_Tx100_Rx100_FlatFading_bound5e-1_bound5e-1/epoch_1.pt'
    cuda = False
