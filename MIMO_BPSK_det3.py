...
def main():
    ...
    for dd in range(0, SNRdB_range.shape[0]):
        ...
        for bb in tqdm(range(0, num_packets)):
            ...
            if pass_flag1 == 0:
                # if any user fails, use neural network to detect all users
                xhat_indices_array = np.zeros([N, new_length], dtype=int)
                for ll in range(0, new_length):
                    xhat_indices_array[:, ll] = apply_network(y_equalized_array[:, ll], model, cuda=False)
                # examine CRC again
                pass_flag2 = 1
                for kk in range(0, N):
                    SER_uncoded_block[kk, bb] = bit_err_rate(x_indices_array[kk], xhat_indices_array[kk])
                    xhat_symbols = modem.constellation[xhat_indices_array[kk]]
