...
storage_dict = {'SNRdB':[0], 
                ...
                'x_indices': np.expand_dims(np.zeros([H_width, ], dtype=int), axis=0),
                'x_symbols': np.expand_dims(void_x, axis=0),
                'y_symbols': np.expand_dims(void_y, axis=0)}
storage_path = './filtered_data/QAM4_Tx100_Rx100_NoH_bound0e-1.npy'
...
for dd in range(0, SNRdB_range.shape[0]):
    ...
    for bb in tqdm(range(0, num_blocks)):
        ...
        for jj in range(0, block_size):
            ...
            X_Initial = QAM4_optimal(y_symbols, H, constellation, bound=np.array([0, 0]))
            ...
            if checksum > 0:
                ...
                storage_dict['x_indices'] = np.append(storage_dict['x_indices'], np.expand_dims(x_indices, axis=0), axis=0)
                storage_dict['y_symbols'] = np.append(storage_dict['y_symbols'], np.expand_dims(y_symbols, axis=0), axis=0)
...
plt.grid(True)
plt.savefig('./figures/QAM4_Tx100_Rx100_NoH_bound0e-1.png')