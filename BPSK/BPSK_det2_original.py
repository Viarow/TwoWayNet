...
from dataset import BPSK_FilteredDataset
...
def eval_once(storage_path, params, Net, modem, trellis, pass_rate, cuda=False):
    ...
    for k in tqdm(range(1, datalen)):
        ...
        y_symbols = y_array[k]
        #y_tensor = torch.cat((torch.from_numpy(np.real(y_symbols)).float(), 
        #                    torch.from_numpy(np.imag(y_symbols)).float()))
        y_tensor = torch.from_numpy(np.real(y_symbols)).float()
        if cuda:
            y_tensor = y_tensor.cuda()
            xhat_tensor = Net(y_tensor)
            xhat_tensor = xhat_tensor.cpu().data()
        else:
            xhat_tensor = Net(y_tensor)
        
        #xhat_real = xhat_tensor[0 : params['H_width']].detach().numpy()
        #xhat_imag = xhat_tensor[params['H_width'] : 2*params['H_width']].detach().numpy()
...
def train_eval(storage_path, pass_rate, ckpt_load_path, ckpt_save_path, cuda):
    ...
    model = FCNet(in_features=H_height, out_features=H_width)
    if cuda:
        model = model.cuda()
    ...
    training_data = BPSK_FilteredDataset(storage_path)
...
def main():
    ...
    pass_rate = np.array([0.0297, 0.1726, 0.5225, 0.853, 0.982, 0.9993])  
    ckpt_load_path = None
